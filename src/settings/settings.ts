...
import { workspace, ConfigurationTarget } from 'coc.nvim';
...
export function watchSettingsFiles(callback: () => void): coc.Disposable {
...
export async function enableLanguageIdForClosestTarget(
    ...
    if (languageId) {
        ...
        if (coc.workspace.workspaceFolders
            && coc.workspace.workspaceFolders.length
